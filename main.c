...
int gServerSck;
int gClientSck;
char text[] = { 0xE0, 0x07, 0xBF, 0xAA,
0xC3, 0xC5, 0xB2, 0xE2, 0xCA, 0xD4, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00 };
...
int response_jparse(char*json){
	...
	if (root == NULL) 
	{
		FileLog::GetFileLog().rotate_logger()->info("Invalid json text\n");
		retval = -1;
	...
	{
		FileLog::GetFileLog().rotate_logger()->info("Invalid code in heartbeat  json\n");
	...
	if (msg == NULL) 
	{
		FileLog::GetFileLog().rotate_logger()->info("Null msg\n");
		return 0;
	}
	...
	for (i = 0; i < num; i++) 
	{
		FileLog::GetFileLog().rotate_logger()->info("{}", i);
		cmd = cJSON_GetArrayItem(msg, i);
		if (msg == NULL)
		{
			FileLog::GetFileLog().rotate_logger()->info("\'msg\' is empty array");
			return 0;
		}
		...
		if (key == NULL)
		{
			FileLog::GetFileLog().rotate_logger()->info("\'command\' not found in heartbeat ack json\n");
			return 0;
		}
		else
		{
			...
			if (strncmp(key->valuestring, "open", strlen("open")) == 0)
				...
				FileLog::GetFileLog().rotate_logger()->info("remote open\n");
				}
				...
				FileLog::GetFileLog().rotate_logger()->info("remote close\n");
				}
			else
			{
				
				FileLog::GetFileLog().rotate_logger()->info("error command\' not found in heartbeat ack json\n");
				FileLog::GetFileLog().rotate_logger()->info(key->valuestring);
				FileLog::GetFileLog().rotate_logger()->info("\n");
			}
		}
			FileLog::GetFileLog().rotate_logger()->info("\'command\' is key\n");

		reqId = cJSON_GetObjectItem(cmd, "reqId");
		if (reqId == NULL)
		{
			FileLog::GetFileLog().rotate_logger()->info("\'reqId\' not found in heartbeat ack json\n");
			return 0;
		}
		...
		if (key->valuestring != NULL)
		{
			...
			if (strlen(HBCMD_SENDTIME_STR) == keylen
